...
export class SubCategoryApi {
  ...
  create(body: { categoryId: number; name: string; description?: string | null; sizes?: string[]; imageUrl?: string | null }) {
    return this.http.post<SubCategory>(`${base}/subcategories`, body);
  }
  update(id: number, body: { name: string; description?: string | null; isActive: boolean; sizes?: string[]; imageUrl?: string | null }) {
...
export interface OrderStatusCount { status: OrderStatus; count: number; amount: number; }
...
export class OrderApi {
  ...
  statusCounts(filters: OrderFilters = {}): Observable<OrderStatusCount[]> {
    return this.http.get<OrderStatusCount[]>(`${base}/orders/status-counts`, { params: toParams(filters as Record<string, unknown>) });
  }
